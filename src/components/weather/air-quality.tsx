...
const AirQuality = ({ data, locationName }: AirQualityProps) => {
  ...
  > = {
    ...
    2: {
      name: "Fair",
      description:
        "Air quality is acceptable. However, there may be a moderate health concern for a very small number of people who are unusually sensitive.",
      color: "text-yellow-500 bg-yellow-500/10 border-yellow-500/30",
    },
    3: {
      name: "Moderate",
      description:
        "Air quality is moderate. Sensitive groups may experience health effects. The general public is not likely to be affected.",
    ...
    <Card className="flex-1 hover-lift group">
      <CardHeader className="border-b">
        <CardTitle className="animate-fade-in">
          Air Quality Index (AQI) for {locationName}
        </CardTitle>
      </CardHeader>
        ...
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-4 p-4 rounded-lg border border-dashed transition-all duration-300 hover:border-solid hover:shadow-lg hover:scale-[1.02] group/item interactive-scale">
            <div className="flex items-center gap-4">
              <div
                className={cn(
                  "size-12 rounded-full border flex items-center justify-center font-bold text-xl text-white transition-all duration-300 group-hover/item:scale-125 group-hover/item:rotate-12 group-hover/item:shadow-lg relative",
                  aqiInfo.color
                    .replace("text-", "bg-")
                    .replace("/10", "")
                    .replace("border-", "")
              ...
              >
                ...
                <div
                  className={cn(
                    "absolute inset-0 rounded-full blur-xl opacity-0 group-hover/item:opacity-50 transition-opacity duration-300 animate-glow-pulse",
                    aqiInfo.color
                      .replace("text-", "bg-")
                      .replace("/10", "")
                      .replace("border-", "")
              ...
              <div>
                ...
                <h2
                  className={cn(
                    "text-2xl font-bold transition-all duration-300 group-hover/item:scale-105",
                    aqiInfo.color
                  )}
                >
                  {aqiInfo.name}
            ...
            <p className="text-sm text-muted-foreground transition-colors group-hover/item:text-foreground/80">
              {aqiInfo.description}
            </p>
          </div>

          <h3 className="text-lg font-semibold border-b pb-2 animate-fade-in">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            {prominentPollutants.length > 0 ? (
              prominentPollutants.map((pollutant, index) => (
                ...
                >
                  <p className="text-xs font-medium text-muted-foreground truncate group-hover/pollutant:text-foreground transition-colors">
                    {pollutant.description}
                  </p>
                  <p className="text-xl font-semibold transition-all duration-300 group-hover/pollutant:scale-125 group-hover/pollutant:text-primary">
                    {pollutant.value.toFixed(2)}
                  </p>
                  <p className="text-xs font-semibold text-muted-foreground transition-colors group-hover/pollutant:text-foreground">
              ...
              <p className="text-sm text-muted-foreground col-span-full">
                All major pollutant levels are currently at zero or trace amounts.
              </p>
            )}
          </div>
        </div>
